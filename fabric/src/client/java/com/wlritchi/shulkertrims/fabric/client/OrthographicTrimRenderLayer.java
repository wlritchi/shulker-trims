 ...
 *
 * <ul>
 *   <li>In perspective mode: uses vanilla's {@code getArmorTrims(false)} layer
 *   <li>In orthographic mode: uses a custom layer with larger forward Z offset
 * </ul>
 *
...
public final class OrthographicTrimRenderLayer extends RenderPhase {
  ...
  /**
   * Offset distance towards camera for orthographic overlay rendering. Uses the same magnitude as
   * vanilla's VIEW_OFFSET_Z_LAYERING (1/512 ≈ 0.00195 units), but translated towards the camera
   * instead of along a fixed axis.
   */
  private static final float ORTHO_CAMERA_OFFSET = 1.0f / 512.0f;

  /**
   * Custom layering phase that translates towards the camera in world space. Unlike vanilla's
   * VIEW_OFFSET_Z_LAYERING which translates along a fixed axis, this computes the camera's look
   * direction and translates towards it.
   */
  private static final RenderPhase.Layering ORTHO_OVERLAY_LAYERING =
      new RenderPhase.Layering(
  ...
  /**
   * Custom render layer for armor trims with larger forward Z offset. This pushes overlays reliably
   * towards the camera in orthographic mode.
   */
  private static final RenderLayer ARMOR_TRIMS_ORTHO =
      RenderLayer.of(
          ...
          RenderLayer.MultiPhaseParameters.builder()
              .texture(
                  new RenderPhase.Texture(TexturedRenderLayers.ARMOR_TRIMS_ATLAS_TEXTURE, false))
   ...
   * items), use {@link TexturedRenderLayers#getArmorTrims(boolean)} directly, as vanilla's layer
   * works correctly for all item rendering contexts.
   *
