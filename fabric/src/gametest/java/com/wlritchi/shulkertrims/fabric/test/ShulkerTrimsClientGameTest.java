...
/**
 * Client-side game tests for Shulker Trims mod. Tests verify that trim overlays render correctly on
 * both placed shulker boxes and items.
 *
 * <p>These tests use the Fabric Client Game Test API to:
 *
 * <ul>
 *   <li>Create singleplayer worlds
 *   <li>Place trimmed shulker boxes
 *   <li>Take screenshots and compare against golden template images
 * </ul>
 *
 * <p><b>Golden Image Workflow:</b>
 *
 * <ul>
 *   <li>Template images are stored in {@code fabric/src/gametest/resources/templates/}
 *   <li>If a template doesn't exist, the test will save the captured screenshot as the new template
 *   <li>To update templates: delete the existing template and re-run the test
 *   <li>Uses Mean Squared Difference algorithm with 1% tolerance for GPU/driver variations
 * </ul>
 */
...
public class ShulkerTrimsClientGameTest implements FabricClientGameTest {
  ...
  private static final List<Block> ALL_SHULKER_BLOCKS =
      List.of(
  ...
  private static final List<net.minecraft.item.Item> ALL_SHULKER_ITEMS =
      List.of(
          Items.SHULKER_BOX,
  ...
  private static final List<String> ALL_MATERIALS =
      List.of(
          "minecraft:quartz",
          "minecraft:iron",
          "minecraft:netherite",
  ...
  /**
   * Material assignments for each shulker color index, chosen for good contrast. Avoids same-hue
   * combinations like white+iron, lime+emerald, cyan+diamond, etc.
   */
  private static final List<String> CONTRASTING_MATERIALS =
      List.of(
  ...
  /**
   * Mean Squared Difference threshold for screenshot comparison. 0.00001 (0.001%) is strict enough
   * to catch single-texture changes like missing trim overlays while allowing for negligible
   * floating-point rendering differences.
   */
  ...
  /**
   * Baseline test with untrimmed shulker boxes in the same grid layout. This creates the golden
   * template for comparing untrimmed shulkers, used in live modification tests to verify trim
   * removal works.
   */
              ...
              server -> {
                ...
                for (int row = 0; row < GRID_ROWS; row++) {
                  for (int col = 0; col < GRID_COLS; col++) {
                    ...
                    Block shulkerBlock =
                        ALL_SHULKER_BLOCKS.get(shulkerIndex % ALL_SHULKER_BLOCKS.size());

                    // Place the colored shulker block WITHOUT trim
                    world.setBlockState(pos, shulkerBlock.getDefaultState());

                    LOGGER.info(
                        "Grid[{},{}]: {} (no trim)", col, row, shulkerBlock.getName().getString());
  ...
  /**
   * Comprehensive test of all trim patterns, shulker colors, and materials. Places 18 shulker boxes
   * in a 6x3 grid, each with a unique pattern. Uses contrasting material assignments for visibility
   * on each shulker color.
   */
  private void testComprehensiveWorldRendering(ClientGameTestContext context) {
    ...
    try (TestSingleplayerContext singleplayer = context.worldBuilder().create()) {
      ...
      singleplayer
          ...
          .runOnServer(
              server -> {
                    ...
                    if (world.getBlockEntity(pos) instanceof ShulkerBoxBlockEntity be) {
                      if (be instanceof TrimmedShulkerBox trimmedBE) {
                        ...
                        LOGGER.info(
                            ...
                            col,
                            row,
                            shulkerBlock.getName().getString(),
                            pattern,
                            material);
                      }
                    }
                ...
                // Place a barrier block for the player to stand on (ensures consistent camera
                // height)
                // Position in front-right of the grid (positive X, positive Z), looking back at
                // grid
      ...
      // In Minecraft: yaw 0 = south (+Z), yaw 90 = west (-X), yaw 180 = north (-Z), yaw 270 = east
      // (+X)
      // To look northwest (toward -X and -Z), yaw should be around 135 (between west=90 and
      // north=180)
      ...
      assertScreenshotMatchesTemplateHighRes(
          context, "shulker-trim-comprehensive-world", 1920, 1080);
  ...
  /**
   * Comprehensive test of all trim patterns, shulker colors, and materials in a chest GUI. Places a
   * chest with 18 trimmed shulker items and opens it. Uses the same pattern/color/material
   * combinations as the world test.
   */
  private void testComprehensiveInventoryRendering(ClientGameTestContext context) {
    ...
    try (TestSingleplayerContext singleplayer = context.worldBuilder().create()) {
      ...
      singleplayer
          ...
          .runOnServer(
              server -> {
                ...
                if (world.getBlockEntity(chestPos)
                  ...
                  for (int i = 0; i < 18; i++) {
                    ...
                    net.minecraft.item.Item shulkerItem =
                        ALL_SHULKER_ITEMS.get(i % ALL_SHULKER_ITEMS.size());
      ...
      singleplayer
          ...
          .runCommand(
              String.format(
                  "tp @p %d %d %d", chestPos.getX(), chestPos.getY(), chestPos.getZ() + 1));
      context.waitTicks(5);
  ...
  /**
   * Test smithing table GUI with a shulker box trim preview. Places items in the smithing table
   * slots and verifies the preview renders correctly.
   */
  private void testSmithingTablePreview(ClientGameTestContext context) {
    ...
    try (TestSingleplayerContext singleplayer = context.worldBuilder().create()) {
      ...
      singleplayer
          ...
          .runOnServer(
              server -> {
                ...
                world.setBlockState(
                    smithingTablePos.down().south(), Blocks.BARRIER.getDefaultState());
              });
      ...
      singleplayer
          .getServer()
          .runCommand("item replace entity @p hotbar.0 with wild_armor_trim_smithing_template 1");
      singleplayer
          .getServer()
          .runCommand("item replace entity @p hotbar.1 with blue_shulker_box 1");
      ...
      singleplayer
          ...
          .runCommand(
              String.format(
                  "tp @p %d %d %d",
                  smithingTablePos.getX(), smithingTablePos.getY(), smithingTablePos.getZ() + 1));
      context.waitTicks(5);
      ...
      // SmithingScreenHandler slot mapping: 0=template, 1=base, 2=addition, 3=result, 4-30=main
      // inventory, 31-39=hotbar
      context.runOnClient(
          client -> {
   ...
   *
   * <p>Uses Mean Squared Difference algorithm with configurable tolerance to handle minor
   * GPU/driver rendering variations while still catching significant visual regressions.
   *
   * <p>If the template image doesn't exist, the screenshot will be saved as the new template. This
   * enables easy initial template generation by simply running the tests.
   *
  ...
  /**
   * Asserts that a screenshot matches the golden template image at a specified resolution. Higher
   * resolution captures more detail for visual inspection.
   *
   * @param context the client game test context
   * @param templateName the name of the template image (without extension)
   ...
   */
  private void assertScreenshotMatchesTemplateHighRes(
      ClientGameTestContext context, String templateName, int width, int height) {
    TestScreenshotComparisonOptions options =
        TestScreenshotComparisonOptions.of(templateName)
            .withAlgorithm(
                TestScreenshotComparisonAlgorithm.meanSquaredDifference(COMPARISON_THRESHOLD))
  ...
  private void placeAndTrimShulker(
      ServerWorld world, BlockPos pos, String pattern, String material) {
    ...
    if (world.getBlockEntity(pos) instanceof ShulkerBoxBlockEntity be) {
      if (be instanceof TrimmedShulkerBox trimmedBE) {
        ...
        be.markDirty();
        LOGGER.info(
            "Placed trimmed shulker at {}: pattern={}, material={}", pos, pattern, material);
      }
    }
  }
}
