...
/**
 * GameTest tests for Shulker Trims mod functionality. Tests verify that trim data is correctly
 * applied, stored, and preserved through various operations.
 */
public class ShulkerTrimsGameTest {
  ...
  /**
   * Test that the smithing recipe correctly applies trim data to a shulker box. Verifies the output
   * ItemStack has correct custom_data with pattern and material.
   */
  @GameTest
  public void testSmithingRecipeAppliesTrim(TestContext context) {
    ...
    ShulkerTrimRecipe recipe =
        new ShulkerTrimRecipe(
    ...
    context.assertTrue(
        result.isOf(Items.SHULKER_BOX), Text.literal("Result should be a shulker box"));
    ...
    context.assertTrue(
        WILD_PATTERN.equals(trim.pattern()),
        Text.literal(
            "Trim pattern should be 'minecraft:wild', got: "
                + (trim != null ? trim.pattern() : "null")));
    context.assertTrue(
        COPPER_MATERIAL.equals(trim.material()),
        Text.literal(
            "Trim material should be 'minecraft:copper', got: "
                + (trim != null ? trim.material() : "null")));
  ...
  public void testBlockPlacementPreservesTrim(TestContext context) {
    ...
    context.assertTrue(be != null, Text.literal("Block entity should be ShulkerBoxBlockEntity"));
    ...
    if (be instanceof TrimmedShulkerBox trimmedBE) {
      ...
      context.assertTrue(
          WILD_PATTERN.equals(storedTrim.pattern()),
          Text.literal("Block entity pattern should match"));
      context.assertTrue(
          COPPER_MATERIAL.equals(storedTrim.material()),
          Text.literal("Block entity material should match"));
    } else {
      context.throwPositionedException(
          Text.literal("Block entity should implement TrimmedShulkerBox"), pos);
    }
  ...
  /**
   * Test a full round-trip: create trimmed item, place, retrieve trim from BE, break, check dropped
   * item. This is a more comprehensive integration test.
   */
  @GameTest
  public void testRoundTripTrimPreservation(TestContext context) {
    ...
    if (be instanceof TrimmedShulkerBox trimmedBE) {
      ...
      context.assertTrue(
          WILD_PATTERN.equals(beTrim.pattern()), Text.literal("BE pattern should match"));
      context.assertTrue(
          COPPER_MATERIAL.equals(beTrim.material()), Text.literal("BE material should match"));
    }
    ...
    context.assertTrue(
        WILD_PATTERN.equals(readBack.pattern()),
        Text.literal("NBT pattern should survive round-trip"));
    context.assertTrue(
        COPPER_MATERIAL.equals(readBack.material()),
        Text.literal("NBT material should survive round-trip"));
  ...
  public void testTrimStorageItemOperations(TestContext context) {
    ...
    // Initially no trim
    context.assertTrue(
        ShulkerTrimStorage.readTrimFromItem(stack) == null,
    ...
    context.assertTrue(
        WILD_PATTERN.equals(readTrim.pattern()), Text.literal("Pattern should match"));
    context.assertTrue(
        COPPER_MATERIAL.equals(readTrim.material()), Text.literal("Material should match"));
  ...
  public void testDifferentTrimMaterials(TestContext context) {
    ...
    context.assertTrue(
        "minecraft:gold".equals(trim.material()),
  ...
  public void testColoredShulkerBox(TestContext context) {
    ...
    context.assertTrue(
        result.isOf(Items.BLUE_SHULKER_BOX), Text.literal("Result should preserve shulker color"));

    ShulkerTrim trim = ShulkerTrimStorage.readTrimFromItem(result);
    context.assertTrue(trim != null, Text.literal("Colored shulker should have trim"));
    context.assertTrue(
        WILD_PATTERN.equals(trim.pattern()), Text.literal("Pattern should be correct"));

    context.complete();
  }

  /**
   * Test dispenser placement of trimmed shulker box. Verifies the dispenser mixin correctly
   * transfers trim data.
   */
  @GameTest
  public void testDispenserPlacement(TestContext context) {
    ...
    DispenserBlockEntity dispenserBE =
        context.getBlockEntity(dispenserPos, DispenserBlockEntity.class);
    if (dispenserBE != null) {
  ...
  private void testRecipeWithPattern(
      TestContext context, Item templateItem, String expectedPattern) {
    ...
    ShulkerTrim trim = ShulkerTrimStorage.readTrimFromItem(result);
    context.assertTrue(
        trim != null, Text.literal("Result should have trim for pattern: " + expectedPattern));
    context.assertTrue(
        expectedPattern.equals(trim.pattern()),
        Text.literal(
            "Pattern should be "
                + expectedPattern
                + ", got: "
