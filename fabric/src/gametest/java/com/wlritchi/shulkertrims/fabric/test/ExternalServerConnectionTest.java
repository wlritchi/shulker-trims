 ...
 *
 * <p>These tests verify that the gametest client can connect to external servers and take
 * screenshots for golden image comparison.
 *
 * <p>Paper tests share a single server instance for efficiency (Paper startup is slow). Use the
 * {@code shulker_trims.paper_tests} system property to control which tests run:
 *
 * <ul>
 *   <li>Unset or empty: Run all Paper tests
 *   <li>Comma-separated list: Run only specified tests (e.g., "world,chest_gui")
 * </ul>
 *
 * <p>Available Paper test names:
 *
 * <ul>
 *   <li>{@code world} - Basic world rendering with trimmed shulkers
 *   <li>{@code chunk_reload} - Chunk unload/reload sync verification
 *   <li>{@code live_modification} - Live trim add/remove while player watches
 *   <li>{@code chest_gui} - Chest GUI with trimmed shulker items
 *   <li>{@code smithing_table} - Smithing table trim preview
 *   <li>{@code dispenser} - Dispenser-placed trimmed shulker sync
 *   <li>{@code two_player} - Two-player placement: bot places block, observer sees trim
 * </ul>
 *
 * <p>Additional test modes (not part of Paper test suite):
 *
 * <ul>
 *   <li>Set {@code shulker_trims.test_mode=fabric} for Fabric in-process server test
 *   <li>Set {@code shulker_trims.test_mode=manual} for manual external server connection
 * </ul>
 */
...
public class ExternalServerConnectionTest implements FabricClientGameTest {
  ...
  private static final Set<String> ALL_PAPER_TESTS =
      Set.of(
          TEST_WORLD,
          TEST_CHUNK_RELOAD,
          TEST_LIVE_MODIFICATION,
          TEST_CHEST_GUI,
          TEST_SMITHING_TABLE,
          TEST_DISPENSER_PLACEMENT,
          TEST_TWO_PLAYER,
          TEST_RESYNC_AFTER_BREAK);

  /**
   * Get the set of Paper tests to run based on system property. If property is unset or empty,
   * returns all tests. Otherwise, parses comma-separated list of test names.
   */
  ...
  /**
   * Run all enabled Paper tests with a shared server instance. This avoids the ~30s Paper startup
   * overhead for each test. Results are reported to {@link TestResultCollector} for final summary.
   */
  private void runPaperTests(ClientGameTestContext context) {
    ...
    try (TestServerLauncher launcher =
        new TestServerLauncher(TestServerLauncher.ServerType.PAPER, TEST_SERVER_PORT)) {
  ...
  /** Clean up the test world via RCON. Removes all blocks and resets to barrier floor. */
  ...
  private void runPaperWorldTest(
      ClientGameTestContext context, String address, TestServerLauncher launcher) {
  ...
  private void runPaperChunkReloadTest(
      ClientGameTestContext context, String address, TestServerLauncher launcher) {
  ...
  private void runPaperLiveModificationTest(
      ClientGameTestContext context, String address, TestServerLauncher launcher) {
  ...
  private void runPaperChestGuiTest(
      ClientGameTestContext context, String address, TestServerLauncher launcher) {
  ...
  private void runPaperSmithingTableTest(
      ClientGameTestContext context, String address, TestServerLauncher launcher) {
  ...
  private void runPaperDispenserPlacementTest(
      ClientGameTestContext context, String address, TestServerLauncher launcher) {
  ...
  /**
   * Run the Paper two-player placement test with a shared server. A bot player places a trimmed
   * shulker box while the observer client watches.
   */
  private void runPaperTwoPlayerPlacementTest(
      ClientGameTestContext context, String address, TestServerLauncher launcher) {
  ...
  /**
   * Test Paper server connection with two players: a bot places a trimmed shulker, observer sees
   * it.
   *
   * <p>This tests that when one player places a trimmed shulker box, the trim data is correctly
   * synced to other nearby players. This exercises the server's broadcast mechanism for custom data
   * on block placement.
   */
  private void testPaperTwoPlayerPlacementConnection(
      ClientGameTestContext context, String serverAddress, TestServerLauncher launcher) {
    ...
    try (MinecraftBot bot = new MinecraftBot("TrimBot", host, port)) {
      ...
      context.runOnClient(
          client -> {
            ServerInfo serverInfo =
                new ServerInfo("Test Paper Server", serverAddress, ServerInfo.ServerType.OTHER);
      ...
      boolean isObserverConnected =
          context.computeOnClient(client -> client.world != null && client.player != null);
      ...
      String observerName =
          context.computeOnClient(
              client -> client.player != null ? client.player.getGameProfile().name() : "Player0");
      ...
      // The block should be placed at (0, 100, 0) by clicking the top of the floor block at (0, 99,
      // 0)
      LOGGER.info(
          "Bot placing block at ({}, {}, {})...",
          TestWorldSetup.TWO_PLAYER_BLOCK_X,
          TestWorldSetup.TWO_PLAYER_BLOCK_Y,
          TestWorldSetup.TWO_PLAYER_BLOCK_Z);

      org.cloudburstmc.math.vector.Vector3i targetBlock =
          org.cloudburstmc.math.vector.Vector3i.from(
              TestWorldSetup.TWO_PLAYER_BLOCK_X,
              TestWorldSetup.TWO_PLAYER_BLOCK_Y - 1, // The floor block
              TestWorldSetup.TWO_PLAYER_BLOCK_Z);
      bot.placeBlock(
          targetBlock, org.geysermc.mcprotocollib.protocol.data.game.entity.object.Direction.UP);
                ...
                if (client.world != null) {
                  var blockPos =
                      new net.minecraft.util.math.BlockPos(
                          ...
                          TestWorldSetup.TWO_PLAYER_BLOCK_Y,
                          TestWorldSetup.TWO_PLAYER_BLOCK_Z);
                  return client.world.getBlockState(blockPos).getBlock()
                      instanceof net.minecraft.block.ShulkerBoxBlock;
                }
                return false;
              });
          ...
          client -> {
            if (client.world != null) {
              ...
              if (block instanceof net.minecraft.block.ShulkerBoxBlock) {
                ...
                if (blockEntity
                    instanceof com.wlritchi.shulkertrims.fabric.TrimmedShulkerBox trimmed) {
                  var trim = trimmed.shulkerTrims$getTrim();
                  if (trim != null) {
                    LOGGER.info(
                        "Trim present in block entity: pattern={}, material={}",
                        trim.pattern(),
                        trim.material());
                  } else {
      ...
      // Uses same template as dispenser test since it's a single blue shulker with wild/redstone
      // trim
      TestScreenshotComparisonOptions options =
          TestScreenshotComparisonOptions.of("shulker-trim-two-player-placed")
              .withAlgorithm(
                  TestScreenshotComparisonAlgorithm.meanSquaredDifference(COMPARISON_THRESHOLD))
              .withSize(1920, 1080)
              .save();
   ...
   *
   * <p>This test verifies that the plugin's lastKnownTrims tracking properly clears when a shulker
   * box is removed, allowing re-sync when an identical trim is placed at the same location.
   *
   * <p>Tests multiple block removal methods:
   *
   * <ul>
   *   <li>Server /setblock air
   *   <li>Explosion
   * </ul>
   */
  private void runPaperResyncAfterBreakTest(
      ClientGameTestContext context, String address, TestServerLauncher launcher) {
    ...
    try {
      ...
      launcher.sendCommand(
          String.format(
              "fill %d %d %d %d %d %d minecraft:barrier",
              RESYNC_TEST_X - 2,
              RESYNC_TEST_Y - 1,
              RESYNC_TEST_Z - 2,
              RESYNC_TEST_X + 2,
      ...
      launcher.sendCommand(
          String.format("tp @a %d %d %d", RESYNC_TEST_X, RESYNC_TEST_Y + 2, RESYNC_TEST_Z + 3));
      ...
      testResyncAfterRemoval(
          context,
          launcher,
          "setblock %d %d %d air".formatted(RESYNC_TEST_X, RESYNC_TEST_Y, RESYNC_TEST_Z));
  ...
  private void testResyncAfterRemoval(
      ClientGameTestContext context, TestServerLauncher launcher, String removalCommand)
    ...
    boolean firstSyncReceived =
        context.computeOnClient(
            client -> {
              if (client.world == null) return false;
              var pos =
                  new net.minecraft.util.math.BlockPos(RESYNC_TEST_X, RESYNC_TEST_Y, RESYNC_TEST_Z);
    ...
    launcher.sendCommand(
        String.format("setblock %d %d %d air", RESYNC_TEST_X, RESYNC_TEST_Y, RESYNC_TEST_Z));
    context.waitTicks(10);
  }
  ...
  private void testPaperLiveModificationConnection(
      ClientGameTestContext context, String serverAddress, TestServerLauncher launcher) {
    ...
    TestScreenshotComparisonOptions untrimmedOptions =
        TestScreenshotComparisonOptions.of("shulker-untrimmed-world")
            .withAlgorithm(
                TestScreenshotComparisonAlgorithm.meanSquaredDifference(COMPARISON_THRESHOLD))
            .withSize(1920, 1080)
            .save();
    ...
    try {
      for (TestWorldSetup.ShulkerPlacement placement :
          TestWorldSetup.getComprehensiveWorldPlacements()) {
        String dataMergeCommand =
            String.format(
    ...
    TestScreenshotComparisonOptions trimmedOptions =
        TestScreenshotComparisonOptions.of("shulker-trim-comprehensive-world")
            .withAlgorithm(
                TestScreenshotComparisonAlgorithm.meanSquaredDifference(COMPARISON_THRESHOLD))
            .withSize(1920, 1080)
            .save();
    ...
    try {
      ...
      throw new AssertionError(
          "Live trim addition failed: trims not visible after /data merge. "
              + "The Paper plugin needs to detect block entity changes and sync.",
          e);
    }
    ...
    try {
      ...
      throw new AssertionError(
          "Live trim removal failed: trims still visible after /data remove. "
              + "The Paper plugin needs to detect block entity changes and sync.",
          e);
    }
  ...
  private void testPaperChestGuiConnection(
      ClientGameTestContext context, String serverAddress, TestServerLauncher launcher) {
    ...
    String teleportCommand =
        TestWorldSetup.generateGuiTeleportCommand(
          ...
          if (client.player != null && client.interactionManager != null) {
            ...
            client.interactionManager.interactBlock(
                ...
                new net.minecraft.util.hit.BlockHitResult(
                    chestPos.toCenterPos(), net.minecraft.util.math.Direction.UP, chestPos, false));
          }
        });
    ...
    boolean isChestOpen =
        context.computeOnClient(
            client ->
                client.currentScreen
                    instanceof net.minecraft.client.gui.screen.ingame.GenericContainerScreen);
    ...
    TestScreenshotComparisonOptions options =
        TestScreenshotComparisonOptions.of("shulker-trim-comprehensive-inventory")
            .withAlgorithm(
                TestScreenshotComparisonAlgorithm.meanSquaredDifference(COMPARISON_THRESHOLD))
            .save();
    context.assertScreenshotEquals(options);
    LOGGER.info(
        "Screenshot comparison passed: shulker-trim-comprehensive-inventory (Paper server)");
  ...
  private void testPaperSmithingTableConnection(
      ClientGameTestContext context, String serverAddress, TestServerLauncher launcher) {
    ...
    try {
      String teleportCommand =
          TestWorldSetup.generateGuiTeleportCommand(
              playerName,
              TestWorldSetup.SMITHING_TABLE_X,
              TestWorldSetup.SMITHING_TABLE_Y,
              TestWorldSetup.SMITHING_TABLE_Z);
      launcher.sendCommand(teleportCommand);
      launcher.sendCommand("gamemode creative " + playerName);
    } catch (Exception e) {
      LOGGER.warn("Failed to teleport player: {}", e.getMessage());
    ...
    // SmithingScreenHandler slot mapping: 0=template, 1=base, 2=addition, 3=result, 4-30=inventory,
    // 31-39=hotbar
    context.runOnClient(
        client -> {
          if (client.player != null
              ...
              && client.player.currentScreenHandler
                  instanceof net.minecraft.screen.SmithingScreenHandler handler) {
    ...
    // Compare against the golden template
    TestScreenshotComparisonOptions options =
        TestScreenshotComparisonOptions.of("shulker-trim-smithing-preview")
            .withAlgorithm(
                TestScreenshotComparisonAlgorithm.meanSquaredDifference(COMPARISON_THRESHOLD))
            .save();
    context.assertScreenshotEquals(options);
   ...
   *
   * <p>This test verifies that when a dispenser places a trimmed shulker box, the trim data is
   * correctly synced to nearby clients. This exercises a different code path than player placement.
   */
  private void testPaperDispenserPlacementConnection(
      ClientGameTestContext context, String serverAddress, TestServerLauncher launcher) {
    ...
    context.runOnClient(
        client -> {
          if (client.player != null && client.world != null) {
            ...
            var outputPos =
                new net.minecraft.util.math.BlockPos(
                    TestWorldSetup.DISPENSER_X,
                    TestWorldSetup.DISPENSER_OUTPUT_Y,
                    TestWorldSetup.DISPENSER_Z);
            var block = client.world.getBlockState(outputPos).getBlock();
            LOGGER.info(
                "Block at dispenser output ({}, {}, {}): {}",
                TestWorldSetup.DISPENSER_X,
                TestWorldSetup.DISPENSER_OUTPUT_Y,
                TestWorldSetup.DISPENSER_Z,
                net.minecraft.registry.Registries.BLOCK.getId(block));
    ...
    try {
      launcher.sendCommand(
          String.format(
              ...
              TestWorldSetup.DISPENSER_X + 1,
              TestWorldSetup.DISPENSER_Y,
              TestWorldSetup.DISPENSER_Z));
      launcher.sendCommand(
          String.format(
              "setblock %d %d %d minecraft:air",
    ...
    context.runOnClient(
        client -> {
          if (client.world != null) {
            ...
            LOGGER.info(
                "Block at dispenser output after firing: {}",
                net.minecraft.registry.Registries.BLOCK.getId(block));
    ...
    TestScreenshotComparisonOptions options =
        TestScreenshotComparisonOptions.of("shulker-trim-dispenser-placed")
            .withAlgorithm(
                TestScreenshotComparisonAlgorithm.meanSquaredDifference(COMPARISON_THRESHOLD))
            .withSize(1920, 1080)
            .save();
  ...
  private void setupWorldViaRcon(TestServerLauncher launcher) {
    try {
      ...
      try {
        ...
        // Expected format: {"shulker_trims:trim": {material: "minecraft:quartz", pattern:
        // "minecraft:sentry"}}
      } catch (Exception e) {
  ...
  /**
   * Test using Fabric's built-in dedicated server creation. This should work - it's the supported
   * approach.
   */
  ...
  /**
   * Test connecting to an external server. Uses the same approach as Fabric's
   * TestDedicatedServerContextImpl.
   *
   ...
   */
  private void testPaperServerConnection(
      ClientGameTestContext context, String serverAddress, TestServerLauncher launcher) {
    ...
    context.runOnClient(
        client -> {
          if (client.player != null) {
            LOGGER.info(
                "Position after teleport: {}, {}, {} (expected: {}, {}, {})",
                client.player.getX(),
                client.player.getY(),
                client.player.getZ(),
                TestWorldSetup.CAMERA_X + 0.5,
                TestWorldSetup.CAMERA_Y,
                TestWorldSetup.CAMERA_Z + 0.5);
          }
        });
    ...
    context.runOnClient(
        client -> {
          if (client.player != null) {
            LOGGER.info(
                "Final position before screenshot: {}, {}, {} (yaw={}, pitch={})",
                client.player.getX(),
                client.player.getY(),
                client.player.getZ(),
   ...
   *
   * <p>This verifies that trim data is synced when player re-enters an area after chunks have been
   * unloaded from the client.
   */
  private void testPaperChunkReloadConnection(
      ClientGameTestContext context, String serverAddress, TestServerLauncher launcher) {
    ...
    LOGGER.info(
        "Phase 2: Teleporting far away to ({}, {}, {}) to force chunk unload",
        farX,
        TestWorldSetup.CAMERA_Y,
        farZ);
    ...
    try {
      String teleportFar =
          String.format("tp %s %d %d %d", playerName, farX, TestWorldSetup.CAMERA_Y, farZ);
    ...
    context.runOnClient(
        client -> {
          if (client.player != null) {
            LOGGER.info(
                "Player position after far teleport: {}, {}, {}",
                client.player.getX(),
                client.player.getY(),
                client.player.getZ());
          }
    ...
    context.runOnClient(
        client -> {
          if (client.player != null) {
            LOGGER.info(
                "Player position after return teleport: {}, {}, {} (expected: {}, {}, {})",
                client.player.getX(),
                client.player.getY(),
                client.player.getZ(),
                TestWorldSetup.CAMERA_X + 0.5,
    ...
    try {
      ...
      throw new AssertionError(
          "Chunk reload sync failed: trims not visible after returning to area. "
              + "The Paper plugin needs to implement PlayerChunkLoadEvent handling.",
          e);
    }
  ...
  /**
   * Wait for world to load, handling any warning screens. Based on Fabric's
   * ClientGameTestImpl.waitForWorldLoad().
   */
      ...
      boolean finished =
          context.computeOnClient(
              client -> {
                ...
                return worldLoaded
                    && !screenName.contains("Loading")
                    && !screenName.contains("Progress");
              });
  ...
  /**
   * Asserts that a screenshot matches the golden template image. Uses Mean Squared Difference
   * algorithm with configurable threshold.
   *
  ...
  private void assertScreenshotMatchesTemplate(ClientGameTestContext context, String templateName) {
    TestScreenshotComparisonOptions options =
        TestScreenshotComparisonOptions.of(templateName)
            .withAlgorithm(
                TestScreenshotComparisonAlgorithm.meanSquaredDifference(COMPARISON_THRESHOLD))
