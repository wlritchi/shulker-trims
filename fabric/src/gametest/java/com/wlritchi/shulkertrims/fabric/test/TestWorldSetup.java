 ...
 *
 * <p>This ensures that both Fabric singleplayer tests and cross-platform server tests use identical
 * world setups, allowing golden screenshots to be compared across platforms.
 */
public final class TestWorldSetup {
  ...
  public static final List<String> ALL_PATTERNS =
      List.of(
          "minecraft:sentry",
          "minecraft:vex",
          "minecraft:wild",
  ...
  public static final List<String> ALL_SHULKER_BLOCKS =
      List.of(
  ...
  public static final List<String> CONTRASTING_MATERIALS =
      List.of(
  ...
  /**
   * Generate commands to set up the test world. These commands can be run on any server (Fabric or
   * Paper) via RCON or console.
   *
  ...
  public static List<String> generateSetupCommands() {
    ...
    commands.add(
        String.format("setblock %d %d %d minecraft:barrier", PLATFORM_X, PLATFORM_Y, PLATFORM_Z));
  ...
  /**
   * Generate basic setup commands (no trims) for testing world setup. Useful for verifying RCON
   * connectivity before adding complex trim data.
   *
   * @return List of commands to execute in order
   */
  ...
  /**
   * Generate commands to set up a chest filled with trimmed shulker boxes. Uses the same
   * pattern/color/material combinations as the world test.
   *
   * @return List of commands to execute in order
   */
  public static List<String> generateChestGuiSetupCommands() {
    ...
    commands.add(
        String.format(
            "fill %d %d %d %d %d %d minecraft:barrier",
            CHEST_X - 1, CHEST_Y - 1, CHEST_Z - 1, CHEST_X + 1, CHEST_Y - 1, CHEST_Z + 2));
  ...
  /**
   * Generate commands to set up a smithing table with trim preview items. Uses blue shulker box +
   * wild pattern + redstone material for high contrast.
   *
  ...
  public static List<String> generateSmithingTableSetupCommands(String playerSelector) {
    ...
    commands.add(
        String.format(
            "setworldspawn %d %d %d", SMITHING_TABLE_X, SMITHING_TABLE_Y, SMITHING_TABLE_Z + 1));
    ...
    commands.add(
        String.format(
            "setblock %d %d %d minecraft:smithing_table",
    ...
    commands.add(
        "item replace entity "
            + playerSelector
            + " hotbar.0 with wild_armor_trim_smithing_template 1");
  ...
  /**
   * Generate commands to set up a dispenser with a trimmed shulker box. Uses blue shulker box +
   * wild pattern + redstone material for visibility.
   *
   * <p>The dispenser is placed facing UP, so when activated it will place a shulker box on the
   * block above it (upright orientation). This tests the Paper plugin's sync mechanism for
   * dispenser-placed blocks.
   *
   * @return List of commands to execute in order
   */
  public static List<String> generateDispenserSetupCommands() {
    ...
    // Set world spawn near test area so chunks load
    commands.add(
        String.format("setworldspawn %d %d %d", DISPENSER_X, DISPENSER_OUTPUT_Y, DISPENSER_Z));
    ...
    commands.add(
        String.format(
            "fill %d %d %d %d %d %d minecraft:barrier",
            DISPENSER_CAMERA_X - 1,
            DISPENSER_CAMERA_Y - 1,
            DISPENSER_CAMERA_Z - 1,
            DISPENSER_CAMERA_X + 1,
            DISPENSER_CAMERA_Y - 1,
            DISPENSER_CAMERA_Z + 1));
    ...
    commands.add(
        String.format(
            "setblock %d %d %d minecraft:dispenser[facing=up]",
  ...
  public static String generateFireDispenserCommand() {
    ...
    return String.format(
        "setblock %d %d %d minecraft:redstone_block", DISPENSER_X + 1, DISPENSER_Y, DISPENSER_Z);
  }
  ...
  public static String generateDispenserTeleportCommand(String playerSelector) {
    return String.format(
        ...
        DISPENSER_CAMERA_Y,
        DISPENSER_CAMERA_Z,
        DISPENSER_CAMERA_YAW,
        DISPENSER_CAMERA_PITCH);
  }
  ...
  /**
   * Generate commands to set up the two-player placement test. Sets up floor, forceloads chunks,
   * but does NOT give items to the bot. The bot's items should be given after it connects.
   *
   * @return List of commands to execute in order
   */
  public static List<String> generateTwoPlayerSetupCommands() {
    ...
    commands.add(
        String.format(
            "setworldspawn %d %d %d", TWO_PLAYER_BLOCK_X, TWO_PLAYER_BLOCK_Y, TWO_PLAYER_BLOCK_Z));

    // Place barrier floor for placement area
    commands.add(
        String.format(
            "fill %d %d %d %d %d %d minecraft:barrier",
            TWO_PLAYER_BLOCK_X - 2,
            TWO_PLAYER_BLOCK_Y - 1,
            TWO_PLAYER_BLOCK_Z - 3,
            TWO_PLAYER_BLOCK_X + 2,
            TWO_PLAYER_BLOCK_Y - 1,
            TWO_PLAYER_BLOCK_Z + 4));

    // Place barrier platform for observer at camera position
    commands.add(
        String.format(
            "fill %d %d %d %d %d %d minecraft:barrier",
            TWO_PLAYER_OBSERVER_X - 1,
            TWO_PLAYER_OBSERVER_Y - 1,
            TWO_PLAYER_OBSERVER_Z - 1,
            TWO_PLAYER_OBSERVER_X + 1,
            TWO_PLAYER_OBSERVER_Y - 1,
            TWO_PLAYER_OBSERVER_Z + 1));
  ...
  /**
   * Generate command to give the placer bot a trimmed shulker box. Uses blue shulker box + wild
   * pattern + redstone material for visibility.
   *
  ...
  public static String generateTwoPlayerObserverTeleportCommand(String playerSelector) {
    return String.format(
        "tp %s %d %d %d %.1f %.1f",
        playerSelector,
        TWO_PLAYER_OBSERVER_X,
        TWO_PLAYER_OBSERVER_Y,
        TWO_PLAYER_OBSERVER_Z,
        TWO_PLAYER_OBSERVER_YAW,
        TWO_PLAYER_OBSERVER_PITCH);
  }
  ...
  public static List<String> generatePlayerSetupCommands(String playerSelector) {
    return List.of("gamemode creative " + playerSelector, generateTeleportCommand(playerSelector));
  }

  /**
   * Generate commands to clean up/reset the test world. Removes all blocks in the test area,
   * restoring it to air/barrier floor. Should be called between tests to ensure a clean state.
   *
  ...
  public static List<String> generateCleanupCommands(String playerSelector) {
    ...
    commands.add(
        String.format(
            "fill %d %d %d %d %d %d minecraft:air", minX, BASE_Y, minZ, maxX, maxY, maxZ));
    ...
    commands.add(
        String.format(
            "tp %s %d %d %d 0 0", playerSelector, PLATFORM_X, PLATFORM_Y + 1, PLATFORM_Z));
