...
/**
 * A simple Minecraft bot client using MCProtocolLib for testing multi-player scenarios. This bot
 * can connect to a server, receive items, and place blocks.
 *
 * <p>This bot properly handles:
 *
 * <ul>
 *   <li>Teleport confirmations (required for the server to accept subsequent packets)
 *   <li>Inventory tracking (to know what items we have)
 *   <li>Position tracking (to confirm we're at the right location)
 * </ul>
 */
public class MinecraftBot implements AutoCloseable {
  ...
  /**
   * Handle server teleport. The server sends this when we're teleported via RCON or spawn. We MUST
   * respond with ServerboundAcceptTeleportationPacket or the server ignores all subsequent packets
   * from us.
   */
  private void handleTeleport(ClientboundPlayerPositionPacket packet) {
    ...
    LOGGER.info(
        "Bot '{}' teleported to ({}, {}, {}) yaw={} pitch={} teleportId={}",
        username,
        x,
        y,
        z,
  ...
  private void handleContainerContent(ClientboundContainerSetContentPacket packet) {
    ...
    if (packet.getContainerId() == 0) {
      ...
      LOGGER.info(
          "Bot '{}' received full inventory update ({} non-empty slots)",
          username,
          inventory.size());
    }
  }
  ...
  /**
   * Send position and rotation to the server. This is necessary before interacting with blocks to
   * confirm client position.
   */
  public void sendPosition(double x, double y, double z, float yaw, float pitch) {
    ...
    LOGGER.info(
        "Bot '{}' sending position ({}, {}, {}) yaw={} pitch={}", username, x, y, z, yaw, pitch);
  ...
  /**
   * Get the item in the currently selected hotbar slot. In player inventory, hotbar slots 0-8 are
   * mapped to inventory slots 36-44.
   */
  ...
  public void placeBlock(Vector3i targetBlockPos, Direction face) {
    ...
    LOGGER.info(
        "Bot '{}' placing block at {} face {} (seq={})", username, targetBlockPos, face, seq);

    // Send the use item on packet (block placement)
    ServerboundUseItemOnPacket usePacket =
