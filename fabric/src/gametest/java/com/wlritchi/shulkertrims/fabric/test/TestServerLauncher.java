...
/**
 * Utility for launching test servers (Fabric dedicated or Paper) from game tests. Servers are
 * started in separate processes and cleaned up automatically.
 */
public class TestServerLauncher implements AutoCloseable {
  ...
  public void start(int timeoutSeconds) throws Exception {
    ...
    if (!waitForPort(port, 10)) {
      throw new RuntimeException(
          "Server port " + port + " not listening after server claimed ready");
    }
  ...
  private void startPaperServer() throws Exception {
    ...
    // Build command
    ProcessBuilder pb =
        new ProcessBuilder("java", "-Xmx1G", "-Xms512M", "-jar", "paper.jar", "--nogui");
    pb.directory(serverDir.toFile());
    pb.redirectErrorStream(true);
  ...
  private void writeServerProperties() throws IOException {
    ...
    String generatorSettings =
        """
                {"layers":[{"block":"minecraft:barrier","height":1}],"biome":"minecraft:the_void","features":false}"""
            .trim();
  ...
  private Path downloadPaperServer(String mcVersion) throws Exception {
    ...
    URL buildsUrl =
        new URL("https://api.papermc.io/v2/projects/paper/versions/" + mcVersion + "/builds");
  ...
  private Path findBukkitPluginJar() {
    ...
    if (!Files.exists(bukkitBuild)) {
      throw new RuntimeException(
          "Bukkit plugin build directory not found: "
              + bukkitBuild.toAbsolutePath()
          ...
          try (BufferedReader reader =
            ...
            while ((line = reader.readLine()) != null) {
              ...
              if (line.contains("FAILED")
                  || line.contains("ERROR") && line.contains("Unable to start")) {
                serverFailed = true;
              }
            }
          } catch (IOException e) {
