 ...
 *
 * <p>RCON (Remote Console) is a protocol for remotely executing commands on game servers. This
 * implementation supports the standard Source RCON protocol used by Minecraft.
 */
public final class RconClient {
   ...
   */
  public static String sendCommand(String host, int port, String password, String command)
      throws IOException {
    try (Socket socket = new Socket(host, port);
      ...
      if (authResponse.requestId != authRequestId) {
        throw new IOException(
            "RCON authentication failed - unexpected request ID: " + authResponse.requestId);
      }
      ...
      if (commandResponse.requestId != commandRequestId) {
        throw new IOException(
            "RCON command failed - unexpected request ID: " + commandResponse.requestId);
      }
  ...
  private static void sendPacket(DataOutputStream out, int requestId, int type, String payload)
      throws IOException {
