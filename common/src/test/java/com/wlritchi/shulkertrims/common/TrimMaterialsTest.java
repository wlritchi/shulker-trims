...
class TrimMaterialsTest {
  ...
  class GetColorTests {
    ...
    @ValueSource(strings = {"unknown", "othermod:copper", "", "minecraft:not_a_trim_material"})
  ...
  class GetColorComponentsTests {
    ...
    void returnsComponentsInValidRange() {
      for (String material : TrimMaterials.COLORS.keySet()) {
        ...
        for (int i = 0; i < 3; i++) {
          assertTrue(
              components[i] >= 0.0f && components[i] <= 1.0f,
    ...
    void modifyingArrayDoesNotAffectFutureCalls() {
      ...
      float[] components2 = TrimMaterials.getColorComponents("minecraft:gold");
      assertEquals(
          originalRed, components2[0], 0.001f, "Modification should not affect future calls");
    }
  }
